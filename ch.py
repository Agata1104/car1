...
class DigitRecognizer:
    def __init__(self, learning_rate=0.01, random_state=42):
        self.learning_rate = learning_rate    # скорость
        self.random_state = random_state     # воспроизводимость результатов
        self.layer1 = None      # веса
        self.layer2 = None
        self.scaler = StandardScaler()
        self.is_trained = False

    def activation(self, x):
        return torch.sigmoid(x)   # преобразует значения в диапазон (0, 1)

    def _one_hot_encode(self, y):
        return torch.eye(10)[y]    # преобразует цифру в вектор

    def loss(self, result, y):
        target = self._one_hot_encode(y)     # целевой вектор
        return torch.sum((result - target) ** 2)

    def _gradient(self, result, x, hidden_output, y):
        target = self._one_hot_encode(y)
        delta_output = (result - target) * result * (1 - result)   # ошибка на выходе
        grad_layer2 = torch.outer(delta_output, hidden_output)   # замена веса layer2 между скрытым и выходным слоем

        delta_hidden = (self.layer2.t() @ delta_output) * hidden_output * (1 - hidden_output)  # ошибка скрытого слоя
        grad_layer1 = torch.outer(delta_hidden, x)    # замена веса layer1 между входом и скрытым слоем
